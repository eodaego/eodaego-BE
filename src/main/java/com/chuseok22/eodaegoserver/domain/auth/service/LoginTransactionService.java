...
public class LoginTransactionService {
  ...
  public LoginResponse login(LoginRequest request, FirebaseToken firebaseToken) {
    Member member = memberRepository.findBySocialTypeAndProviderId(request.socialType(), firebaseToken.getUid()).orElseGet(() -> createMember(request, firebaseToken));
    ...
    log.info("로그인 성공: memberId={}, firstLogin={}, requiresAgreement={}", member.getId(), firstLogin, requiresAgreement);

    return LoginResponse.of(tokenResponse, requiresAgreement, member);
  }

  private Member createMember(LoginRequest request, FirebaseToken firebaseToken) {
    Member newMember = Member.builder().email(firebaseToken.getEmail()).nickname(randomNicknameGenerator.generate()).socialType(request.socialType()).providerId(firebaseToken.getUid()).role(Role.USER)
      .firstLogin(true).deviceType(request.deviceType()).deviceId(request.deviceId()).fcmToken(request.fcmToken()).build();

    return memberRepository.saveAndFlush(newMember);
  }

  private boolean hasAgreedRequiredTerms(Member member) {
    return member.isPrivacyPolicyAgreed() && member.isLocationInfoAgreed() && member.isTermsOfServiceAgreed();
  }
  ...
  private TokenResponse issueTokens(Member member, boolean firstLogin) {
    ...
    refreshTokenRepository.findByMember(member).ifPresentOrElse(existing -> {
      existing.setToken(refreshToken);
      existing.setExpiryDate(expiryDate);
    }, () -> refreshTokenRepository.save(RefreshToken.builder().member(member).token(refreshToken).expiryDate(expiryDate).build()));
