...
- **Entity**: `@Getter @Setter @Builder`, 도메인/팩토리 메서드 없이 Builder+Setter로만 생성/변경. `BaseEntity` 상속, PK는 UUID, 연관관계는 JPA 어노테이션, Enum은 STRING, boolean 필드는 `is` 접두사 없이.
...
- **절대 금지**: `application-*.yml`(dev/prod) git 커밋, `firebase-adminsdk.json` 등 자격증명 커밋, 축약어 사용, Entity에 비즈니스 로직 추가, `application.yml`의 `admin`/`jwt`/`firebase` 관련 값은 실제 값이 아닌 예시 값 그대로 커밋.
